...
void init_set_v(double **set, hmm_seq *seq) {
    if (!seq)
        return;
...
void init_gammaset(double ***gammaset, hmm_seq *seq, hmm_model *model) {
    if(!seq || !model)
        return;
    ...
    for (qword t = 0; t < seq->T - 1; ++t) {
        for (byte i = 0; i < model->N; ++i) {
            for (byte j = 0; j < model->N; ++j) {
                ...
                sum += log2(betaset[t][i]); // maybe t+1, j
                for (qword teta = t; teta < seq->T - 1; ++teta)
                    sum += log2(betaset_v[teta + 1]); // + 1 ?! need?
                sum -= log2(estimation_seq);
...
void estimation_model(hmm_seq *seq, hmm_model *model, double eps){
    ...
    while(1){
        ...
        marginaol_probability_norm(seq, model, est, alphaset, alphaset_v, betaset, betaset_v, gammaset);
        double_probability_norm(seq, model, est, alphaset, alphaset_v, betaset, betaset_v, ksiset);
