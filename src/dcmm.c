...
void init_dcmm_model(dcmm_model *ctx, byte n, byte m){
    ...
    for (byte i = 0; i < ctx->N; ++i) {
        ...
        for (byte j = 0; i < ctx->M; ++i) {
            _memcheck(ctx->C[i][j], ctx->M * sizeof(double));
        }
    }
}
...
void copy_dcmm_model(dcmm_model *dist, dcmm_model *src){
    ...
    for (byte k = 0; k < src->N; ++k) {
        for (int j = 0; j < src->M; ++j) {
            for (int i = 0; i < src->M; ++i) {
                dist->C[k][j][i] = src->C[k][j][i];
            }
        }
    }
}
...
byte generate_dcmm_model(dcmm_model *model,  uint8_t type, uint32_t* param){
    ...
    if(type == 0) {
        ...
        for (byte k = 0; k < model->N; ++k) {
            for (byte i = 0; i < model->M; ++i) {
                ...
                for (byte j = 0; j < model->M; ++j)
                    model->C[k][i][j] = (double) arr[j] / sum;
                free(arr);
            }
        }
    }else if(type == 1) {
        ...
        for (byte k = 0; k < model->N; ++k) {
            for (byte i = 0; i < model->M; ++i) {
                for (byte j = 0; j < model->M; ++j)
                    model->C[k][i][j] = (double) 1 / model->M;
            }
        }
    }
//    }else{
...
byte forward_algorithm_d(sequence *seq, dcmm_model *model, double **set, double *set_v){
    ...
    for (byte i = 0; i < model->N; ++i) {
        alpha[i] = model->C[i][seq->array[0]][seq->array[1]] * model->Pi[i];
        if(isnan(alpha[i])){
            printf("adsd");
        }
        alpha_v += alpha[i];
    ...
    for (qword t = 2; t < seq->T; ++t) {
        ...
        for (byte i = 0; i < model->N; ++i) {
            ...
            alpha[i] = model->C[i][seq->array[t-1]][seq->array[t]] * sum;
            alpha_v += alpha[i];
        }
        alpha_v /= model->N;
...
byte backward_algorithm_d(sequence *seq, dcmm_model *model, double **set, double *set_v){
    ...
    for (qword t = seq->T - 2; t >= 2; --t) {
        beta_v = 0;
        for (byte i = 0; i < model->N; ++i) {
            double sum = 0;
            for (byte j = 0; j < model->N; ++j) {
                sum += model->P[i][j] * model->C[j][seq->array[t]][seq->array[t + 1]]  * beta_set[t][j];
            }
            beta_set[t - 1][i] = sum;
    ...
    //t = 1
    {
        ...
        for (byte i = 0; i < model->N; ++i) {
            double sum = 0;
            for (byte j = 0; j < model->N; ++j) {
                sum += model->P[i][j] * model->C[j][seq->array[1]][seq->array[2]] * beta_set[1][j];
            }
            beta_set[0][i] = sum;
            beta_v += beta_set[0][i];
        }
...
//byte backward_algorithm_(sequence *seq, dcmm_model *model, double **set){
//    if(seq == NULL || model == NULL || seq->m != model->M)
//        return ERROR;
//    double **beta_set = set;
//
    ...
    for (qword t = 0; t < seq->T - 1; ++t) {
        for (byte i = 0; i < model->N; ++i) {
            for (byte j = 0; j < model->N; ++j) {
                ...
                sum += log2(model->C[j][seq->array[t]][seq->array[t + 1]]);
 ...
 */
void estimation_model_d(sequence *seq, dcmm_model *model_i, double eps, double *likehood) {
    ...
    init_dcmm_model(&model, model_i->N, model_i->M);
    copy_dcmm_model(&model, model_i);
    //generate_random_dcmm_model(&model);
    ...
    while (1) {
        ...
        for (byte k = 0; k < model.N; ++k) {
            for (byte i = 0; i < model.M; ++i) {
                for (byte j = 0; j < model.M; ++j) {
                    double sum1, sum2 = sum1 = 0;
                    for (qword t = 0; t < seq->T; ++t) {
                        if (seq->array[t + 1] == j && seq->array[t - 1] == i)
                            sum1 += gammaset[t][k];
                        if (seq->array[t - 1] == i)
                            sum2 += gammaset[t][k];
                    }
        ...
        copy_dcmm_model(&model, &new_model);
        free_dcmm_model(&new_model);
...
//void estimation_model_gl(sequence *seq, dcmm_model *model_i, uint32_t iter, double eps, double *likehood) {
...
//            dcmm_model model_iter;
//            init_dcmm_model(&model_iter, model_i->N, model_i->M);
//            uint32_t params[4] = {iter, iter, i, j};
//            if(generate_dcmm_model(&model_iter, 2, params) == ERROR)
//                continue;
//
////            printf("NEW MODEL\n");
////            printf("Pi:  ");
...
//            if(est < current_est){
//                copy_dcmm_model(&model, &model_iter);
//                est = current_est;
//            }
//            //   printf("\n\n====================================================\n");
//        }
